-- XMLPrompter Database RPC Functions - FIXED VERSION
-- Enable MCP to execute SQL directly through Supabase
-- Execute this in Supabase SQL Editor to enable MCP SQL execution

-- =============================================================================
-- PART 1: CREATE MIGRATIONS TABLE FIRST
-- =============================================================================

-- Create migrations table before functions that reference it
...
-- =============================================================================
-- PART 2: CORE RPC FUNCTIONS FOR MCP INTEGRATION
-- =============================================================================
...
BEGIN
    ...
    -- For now, we'll allow execution but you can add user role checks here
...
BEGIN
    -- Get current user ID (may be null for service role)
    ...
    BEGIN
        ...
        VALUES (migration_name, sql_query, current_user_id, true)
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        VALUES (migration_name, sql_query, current_user_id, false, SQLERRM)
        RETURNING id INTO migration_id;
...
-- =============================================================================
-- PART 3: DATABASE ANALYSIS FUNCTIONS FOR MCP
-- =============================================================================
...
-- =============================================================================
-- PART 4: UTILITY FUNCTIONS FOR DATABASE MANAGEMENT
-- =============================================================================
...
-- =============================================================================
-- PART 5: GRANT PERMISSIONS FOR MCP ACCESS
-- =============================================================================
...
-- Also grant to anon for service role access
GRANT EXECUTE ON FUNCTION public.exec_sql(text) TO anon;
GRANT EXECUTE ON FUNCTION public.exec_query(text) TO anon;
GRANT EXECUTE ON FUNCTION public.exec_migration(text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.get_database_stats() TO anon;
GRANT EXECUTE ON FUNCTION public.analyze_query_performance(text) TO anon;
GRANT EXECUTE ON FUNCTION public.create_index_safe(text, text, text, text) TO anon;
GRANT EXECUTE ON FUNCTION public.cleanup_old_data(text, text, integer) TO anon;
...
SELECT public.exec_query('SELECT ''RPC Functions Working!'' as test_result, now() as timestamp') as test; 